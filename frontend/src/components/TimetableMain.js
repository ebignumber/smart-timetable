...
function TimetableMain() {
  ...
  const deleteTeacher = (name) =>
    setTeachers(teachers.filter(t => t.name !== name));
  ...
  const addClass = (name) => {
    ...
    if (!timetable[name]) {
      setTimetable({
        ...timetable,
        [name]: Object.keys(periodTimings).reduce(
          (acc, slot) => ({ ...acc, [slot]: {} }),
          {}
        ),
      });
  ...
  const updateCell = (className, period, day, data) => {
    for (let c in timetable) {
      ...
      if (
        timetable[c][period] &&
        timetable[c][period][day] &&
        timetable[c][period][day].teacher === data.teacher
      ) {
        alert(
          `Conflict: Teacher ${data.teacher} is already assigned to ${c} at ${period} on ${day}`
        );
        return;
      }
    }
      ...
      for (let period in timetable[className]) {
        for (let day in timetable[className][period]) {
          const { subject, teacher, room } =
            timetable[className][period][day];
          csv += `${className},${period},${day},${subject || ""},${
            teacher || ""
          },${room || ""}\n`;
        }
      }
    ...
    <div className="p-4 bg-gray-100 min-h-screen">
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <PeriodSettings
          ...
          setPeriodTimings={setPeriodTimings}
        />
      </div>
